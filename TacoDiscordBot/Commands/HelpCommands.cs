    ...
    private static readonly IReadOnlyDictionary<string, (string Label, ButtonStyle Style, string Guide)> Categories =
        new Dictionary<string, (string Label, ButtonStyle Style, string Guide)>
        {
            ...
            ["casino"] = ("🎰 カジノ", ButtonStyle.Danger,
                ...
                + "ベット額は1以上かつ所持コイン以内です。\n\n"
                  + "```text\n操作        内容\nHIT         カードを1枚引く\nSTAND       その時点で勝負する\nDOUBLE DOWN 初期2枚のときだけ初期ベットと同額を追加し、1枚だけ引いて自動STAND\nSURRENDER   降参する\n```\n"
                  + "Aは、合計が21を超えない範囲で1または11として扱います。\n"
                  + "DOUBLE DOWNは追加ベット分のコインがあり、最初の2枚でブラックジャックでない場合だけ選択できます。\n"
                  + "DOUBLE DOWN後の21は通常の勝利で、BLACKJACK配当にはなりません。\n"
                  + "SURRENDERは最初の2枚のときだけ選択できます。\n\n"
                ...
                + "`/mines bet:<額>`\n"
                 ...
                 + "ベット額は1以上かつ所持コイン以内です。\n"
                  + "通常の爆弾は4個で、安全マスを開けるほど倍率が上がります。サーバーイベント中は爆弾が減少する場合があります。\n\n"
                  + "・`CHECKOUT`で回収できます。\n"
                 + "・爆弾を開けると払い戻しなしで終了します。\n"
            ...
            ["event"] = ("🔥 サーバーイベント", ButtonStyle.Success,
                ...
                + "「生きるか死ぬか」は個人イベントのため、サーバーイベントと同時に発動できます。\n"
                  + "倍返し、激アツスロット、50% BACK保証、豪運に幸あれ！はサーバー内TOP10の合計コインを基準に計算します。\n"
                  + "ブラックジャック保険、Mines安全週間1～3、生きるか死ぬかは発動者の所持コインを基準に計算します。"),
